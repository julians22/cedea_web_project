...
<div class="${modalOpen ? 'relative w-auto' : '' } h-auto" x-data="{ modalOpen: false }"
    ...
    <template x-teleport="body">
        <div class="fixed left-0 top-0 z-[99] flex h-screen w-screen items-center justify-center" x-show="modalOpen"
            ...
            <div {{ $attributes->withoutTwMergeClasses()->twMerge('relative px-4 py-4 sm:px-7 h-[95%] overflow-clip max-w-[90vw] sm:py-6 sm:max-w-lg rounded-lg bg-cedea-red sm:rounded-3xl') }}
                ...
                <div class="mt-[10%] h-[95%] overflow-auto">
                    @if ($slot)
                        {{ $slot }}
                    @else
